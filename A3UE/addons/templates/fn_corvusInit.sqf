...
// Patch: remove COR_FluidLoss_EH after CORVUS's COR_fnc_damage sets it for GFL AI units.
// ACE_Medical_Injuries handlers fire in registration order; since CORVUS is a dependency
// it registers first, so this handler fires second — after DoFluidLoss has already run.
// This prevents the global EH from trampling between units and saves per-damage CBA overhead.
// Player CORVUS runs on each player's own machine, not the server, so they are unaffected.
if (isNil "GFL_COR_InjuryHook") then {
    GFL_COR_InjuryHook = addMissionEventHandler ["ACE_Medical_Injuries", {
        params ["_unit"];
        if (!isPlayer _unit && _unit getVariable ["GFL_COR_InitDone", false]) then {
...
// Scanner: runs every 10 s, finds newly spawned FCC-equipped units and initialises them.
[{
    params ["_fccPacks"];
    {
        private _unit = _x;
        ...
        if (_unit getVariable ["GFL_COR_InitDone", false]) then { continue };
...
addMissionEventHandler ["EntityCreated", {
    ...
    [{
        ...
        private _targetUniform = GFL_FaceUniformMap getOrDefault [face _unit, ""];
        if (_targetUniform isEqualTo "" || uniform _unit isEqualTo _targetUniform) exitWith {};
